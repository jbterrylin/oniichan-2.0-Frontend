          ...
          <div class="card-body p-0">
            <div class="invoice-container">
              <div class="invoice-header">
                <!-- Row start -->
                <div class="row gutters">
                  <div class="col-xl-9 col-lg-9 col-md-9 col-sm-9 mb-4">
                    <a
                  ...
                  <div class="col-xl-8 col-lg-8 col-md-8 col-sm-8 col-8">
                    <div class="invoice-details">
                      <address>
                        ...
                        >
                        <div class="ml-2">
                          {{ paper && paper.customer.phone
                          }}<br v-if="paper && paper.customer.phone" />
                          {{ paper && paper.customer.name
                          }}<br v-if="paper && paper.customer.name" />
                          {{ paper && paper.customer.ssm }}<br />
                          {{ paper && paper.customer.address }}
                        </div>
                      </address>
                    </div>
                  </div>
                    ...
                    <div class="invoice-details">
                      ...
                      <div class="invoice-num">
                        <v-row :no-gutters="true">
                          ...
                          <v-col cols="7">{{
                            paper && paper.paper_type.toUpperCase()
                          }}</v-col>
                          <v-col cols="5" style="text-align: left">Id: </v-col>
                          <v-col cols="7">#{{ paper && paper.id }}</v-col>
                          <v-col cols="5" style="text-align: left"
                            >Date:
                          </v-col>
                          <v-col cols="7">{{
                          ...
                          <v-col cols="7">{{
                            paper && paper.price_unit.toUpperCase()
                          }}</v-col>
                        </v-row>
                      </div>
                    </div>
                  ...
                  <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="table-responsive">
                      <table class="table custom-table m-0">
                        ...
                        <tbody>
                          <tr
                            v-for="(item, index) in paper && paper.items
                              ? paper.items
                              : []"
                            :key="index"
                          >
                            ...
                            <td>
                              {{
                                item.unit != "" && item.unit_price != ""
                                  ? (item.unit * item.unit_price).toFixed(2)
                                  : null
                              }}
                            </td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td>
                              <p>
                                ...
                                {{ paper && paper.discount }}<br />
                                {{ paper && paper.deposit }}<br />
                              </p>
                              <h5 class="text-success">
    ...
    paperStore()
      ...
      .then((response) => {
        ...
        if (response.status === "ok" && response.data != null) {
          ...
          this.paper.updated_at = new moment(this.paper.updated_at).format(
            "DD/MM/YYYY"
          );
          console.log(
            moment(new Date()).format("ddd MMM DD, YYYY [at] HH:mm a")
          );
        }
      });
  },
    ...
    grandTotal() {
      if (this.paper) {
        ...
        if (this.paper.paper_type == PAPER_TYPE.QUOTATION) {
          ...
          grandTotal = subtotal - this.paper.discount - this.paper.deposit;
        }
        return grandTotal;
      }
      return 0;
